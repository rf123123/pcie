...
//#endif
 ...
 struct task_struct *sendtask;
  ...
  struct task_struct *sfrecvtask;
 struct task_struct *sfsendtask;
wait_queue_head_t sendinq; 			
wait_queue_head_t recvoutq;		
wait_queue_head_t sfsendinq; 			
wait_queue_head_t sfrecvoutq;	
...
volatile UINT 		Last_send = 2;
volatile UINT 		SFLast_send = 0;
...
struct queues{
	...
	dma_addr_t  BufferPh;
	int     len;
} RcvQ[MAXRECVQL], SndQ[MAXSENDQL],RsfQ[MAXENCRYPT];
...
int Other_Side_Recv(void)
{
	...
	if(ret < 256){
			//PRINTK("<Other Side Space> is less than 128   is %d\n",ret);
			ret = 0;
		}
	else
		ret = ret -256;
...
{
	int i,j,n,sendcount,recvhead,recvtail,ret;
	int idx = 0;
	i = 0;
	...
	while(1){
		...
		if( kthread_should_stop()) 
			break;
		if((read_BAR0(DMA_SFSND_CTRL)&DMA_STATUS_BUSY)==0){     //SEND DMA is not working

			for( i = SFLast_send; i <( SFLast_send + 3); i++ ){
				...
				sendcount = Sendnum(idx,recvhead,recvtail,1);
				...
				if(sendcount > 0){
					...
					//PRINTK("after sf dma send \n");							
#if 	0					
					for(j=0;j < sendcount;j++){
						if((pcie56_devs[idx].Send_ktcount +j )!= (*(unsigned int *)(pcie56_devs[idx].devicesendq[(pcie56_devs[idx].STail+j)&MAX_NUM].Buffer+40)))
							PRINTK("pcie56_devs[%d].Send_ktcount is %d,pcie56_devs[%d].devicesendq[%d].Buffer+40) is %d \n",idx,pcie56_devs[idx].Send_ktcount,idx,(pcie56_devs[idx].STail+j)&MAX_NUM,*(unsigned int *)(pcie56_devs[idx].devicesendq[(pcie56_devs[idx].STail+j)&MAX_NUM].Buffer+40));							
					}	
					pcie56_devs[idx].Send_ktcount += sendcount;
				//	PRINTK("%d   Send count is %d\n",idx,pcie56_devs[idx].Send_ktcount);
#endif			

					//  PRINTK("%d   pcie56_devs[idx].STail count is %d\n",idx,pcie56_devs[idx].STail);
					pcie56_devs[idx].Slisttail = pcie56_devs[idx].STail = (pcie56_devs[idx].STail+sendcount)&MAX_NUM; //updata the sendlist header	
		...
		if((read_BAR0(DMA_SND_CTRL)&DMA_STATUS_BUSY)==0){     //SEND DMA is not working
			...
			//PRINTK("<Send_Kthread>dma0 is working!\n");

			//nothing to do
		}
			//end if(DMA is not working)
	add_wait_queue(&sendinq,&wait2);
	__set_current_state(TASK_INTERRUPTIBLE);
	schedule_timeout(20);
	__set_current_state(TASK_RUNNING);
	remove_wait_queue(&sendinq,&wait2);
	}
	//end while 1 
}
...
void recv_thread(void)
{
	...
	while(1){
		...
		spin_unlock_bh(&recvlock);
		while((sf_list[SFTail].status&DMA_RCV_LIST_FLAG)!=0){
			...
			if(((pcie56_devs[id].RHead+1)&MAX_NUM) != pcie56_devs[id].RTail){
				recvbuff = pcie56_devs[id].devicerecvq[pcie56_devs[id].RHead].Buffer;
				memcpy(recvbuff,RsfQ[SFTail].Buffer,recvlen);
#if 0
				if(pcie56_devs[id].Recv_count != (*(unsigned int *)(recvbuff+32+8)))
				{
					PRINTK("%d pcie56_dev[%d].Recv_count is %d     PKG_id is %d  recvlisttail is %d \n",id,id,pcie56_devs[id].Recv_count,*(unsigned int *)(recvbuff+32+8),recvlisttail);
					PRINTK("dev[0].Recvcount is %d, dev[1].Recvcount is %d, dev[2].Recvcount is %d, dev[3].Recvcount is %d, dev[4].Recvcount is %d, dev[5].Recvcount is %d\n",pcie56_devs[0].Recv_count,pcie56_devs[1].Recv_count,pcie56_devs[2].Recv_count,pcie56_devs[3].Recv_count,pcie56_devs[4].Recv_count,pcie56_devs[5].Recv_count);
				}
#endif			
				//PRINTK("pcie56_devs[id].Recv_count is %d\n",pcie56_devs[id].Recv_count);
				pcie56_devs[id].Recv_count++;
		...
		while((recv_list[recvlisttail].status&DMA_RCV_LIST_FLAG)!=0){
			...
			id = *(unsigned int *)(&RcvQ[recvlisttail].Buffer[12]) ;
			...
			if(((pcie56_devs[id].RHead+1)&MAX_NUM) != pcie56_devs[id].RTail){
				...
				recvlisttail = recvlisttail + 1;
				if(recvlisttail>1023){	
					recvlisttail &= MAX_NUM;
					Own_head = (Own_head&0x8000)+recvlisttail +0x8000;
						
				}else{
					Own_head = (Own_head&0x8000)+recvlisttail;
				}
				...
				//PRINTK("<Recv_Kthread>Recv count = %d, pcie56_devs[%d].RHead is %d \n",Recv_count,id,pcie56_devs[id].RHead);
			}
			else{
				spin_unlock_bh(&pcie56_devs[id].readlock);
				break;
...
void set_mytasklet(void *taskleta)
{
	PRINTK("test set tasklet\n");
	DECLARE_TASKLET(recvtasklet,taskleta,0);
 	mytasklet = &recvtasklet;
}
int testflag = 0 ;
void pk_ok()
{
...
//	DECLARE_TASKLET(mytasklet,NULL,0);
...
void send_thread(void)
{
	...
	while(1){
		...
		if( kthread_should_stop()) 
			break;
		while((read_BAR0(DMA_SND_CTRL)&DMA_STATUS_BUSY)==0){     //SEND DMA is not working
			...
			for( i = Last_send; i < Last_send + DEVICE_COUNT; i++ ){
				...
				if(idx > 1){
					...
					if(sendcount > 0){
						...
						pcie56_devs[idx].devicesend[(pcie56_devs[idx].STail+sendcount -1)&MAX_NUM].NextDesc_low |= SND_LIST_END;	//edit the last packet endflag

						start_dma0(idx,pcie56_devs[idx].STail); //start chanel dma
#if 0						
						for(j=0;j < sendcount;j++){
...
#endif	
							pcie56_devs[idx].Slisttail = pcie56_devs[idx].STail = (pcie56_devs[idx].STail+sendcount)&MAX_NUM; //updata the sendlist header	
							wake_up_interruptible(&pcie56_devs[idx].sendoutq);	//wake up can write flag
							spin_unlock_bh(&pcie56_devs[idx].writelock);	
...
void recv_thread(void)
{
	...
	while(1){
		...
		spin_lock_bh(&recvlock);
	 	recvlisttail = Rlisttail;
		spin_unlock_bh(&recvlock);
		while((recv_list[recvlisttail].status&DMA_RCV_LIST_FLAG)!=0){   //check up the recv flag
			//PRINTK("recv_thread habe data \n");
			id = *(unsigned int *)(&RcvQ[recvlisttail].Buffer[12]) ;		//get the device ID
			recvlen= 	recv_list[recvlisttail].status;					//get the recv length
			Change_BELE((unsigned char *)&recvlen);	
			Change_BELE((unsigned char *)&id);
			id &= 0xff; 
			recvlen = recvlen & 0x7fffffff;
			//PRINTK("recvlist is %d\n",recvlisttail);
			spin_lock_bh(&pcie56_devs[id].readlock);
			if(((pcie56_devs[id].RHead+1)&MAX_NUM)/*%MAXRECVQL*/ != pcie56_devs[id].RTail){//check  up the device recvlist space  
				recvbuff = pcie56_devs[id].devicerecvq[pcie56_devs[id].RHead].Buffer;
				memcpy(recvbuff,RcvQ[recvlisttail].Buffer,recvlen);
#if 0				
				if(pcie56_devs[id].Recv_count != (*(unsigned int *)(recvbuff+32+8)))
				...
				pcie56_devs[id].Recv_count++;
#endif
				recv_list[recvlisttail].status = 0;		//	reset the recvflag
				// count the local recvtail and write to fpga 
				recvlisttail = recvlisttail + 1;
				if(recvlisttail>1023){						
					recvlisttail &= MAX_NUM;
					Own_head = (Own_head&0x8000)+recvlisttail +0x8000;
				...
				write_BAR0(RECV_OWN_HEAD, (Own_head&0xffff));
				// upadte the device recvlist
				pcie56_devs[id].devicerecvq[pcie56_devs[id].RHead].len = recvlen;
				pcie56_devs[id].RHead= (pcie56_devs[id].RHead + 1)&MAX_NUM;//%MAXRECVQL;
				//wake up the recvlist
				wake_up_interruptible(&pcie56_devs[id].recvinq);	
				spin_unlock_bh(&pcie56_devs[id].readlock);
...
**********************************************************************/
void sf_recv_thread(void)
{

	int id,recvlen,recvlisttail,i;
	unsigned char* recvbuff;
	//PRINTK("sf_recv_thread: 1\n");
	DECLARE_WAITQUEUE(wait2, current);
	spin_lock_irq(&current->sighand->siglock);
	siginitsetinv(&current->blocked , sigmask(SIGKILL) | sigmask(SIGSTOP));
	recalc_sigpending();
	spin_unlock_irq(&current->sighand->siglock);
	//PRINTK("sf_recv_thread:2 \n");
	while(1){
		//PRINTK("sf_recv_thread:3 \n");
		if( kthread_should_stop()) 
			break;
	/*	
		spin_lock_bh(&recvlock);
...
#endif
	add_wait_queue(&sfrecvoutq,&wait2);
	__set_current_state(TASK_INTERRUPTIBLE);
	schedule_timeout(20);
	__set_current_state(TASK_RUNNING);
	remove_wait_queue(&sfrecvoutq,&wait2);
	}
}
...
void sf_send_thread(void)
{
	...
	while(1){
		...
		if( kthread_should_stop()) 
			break;
		while((read_BAR0(DMA_SFSND_CTRL)&DMA_STATUS_BUSY)==0){     //SEND DMA is not working

			for( i = SFLast_send; i <( SFLast_send + 2); i++ ){
				...
				//	PRINTK("%d   Send count is %d\n",idx,pcie56_devs[idx].Send_ktcount);
					...
					//nothing to do	
						spin_unlock_bh(&pcie56_devs[idx].writelock);	
						if(i==(SFLast_send+2-1))
						{
							add_wait_queue(&sfsendinq,&wait2);
							__set_current_state(TASK_INTERRUPTIBLE);
							schedule_timeout(20);
							__set_current_state(TASK_RUNNING);
							remove_wait_queue(&sfsendinq,&wait2);
...
//	PRINTK("<pcie56_interrupt_send>:send complete interrupt!\n");
	...
	if( IntStat & DMA_SFSND_INT){
//	PRINTK("<pcie56_interrupt_send>:send complete interrupt!\n");
		wake_up_interruptible(&sfsendinq);
	}
...
//		PRINTK("<pcie56_interrupt_recv>:recv complete interrupt!\n");
	...
	if(IntStat & DMA_SFRCV_INT){
		...
		wake_up_interruptible(&sfrecvoutq);
		//spin_unlock_bh(&sflock);
	}
	return IRQ_HANDLED;
}
...
*/
	spin_unlock_bh(&dev->readlock);
#if 0
	if(dev->DeviceID < 2){					// sf data ,they have 32byte header
...
//	PRINTK("%s: id is %d  RTAil is %d  ret is %d\n", __func__,dev->DeviceID,dev->RTail, ret);
	...
	if(dev->DeviceID<3)
		wake_up_interruptible(&sfrecvoutq);			//wake the recv kernel_thread to recv data
	else
...
//	EnterFunction();
//	PRINTK("<data_read>    dev->DeviceID is %d\n",dev->DeviceID);
	if((count==0)||(buf==NULL)){
		return 0;
	}
	spin_lock_bh(&dev->readlock);
...
{	
	...
	wake_up_interruptible(&sfsendinq);											//wake up send kernel-thread to send the data
	spin_unlock_bh(&dev->writelock);
	//LeaveFunction();
    	return count;
}
...
{	
	...
	//
#if 1
//  test sf dma DECLARE_TASKLET(mytasklet,tasklet,0);
	//
		slen = count+32;
...
//	DMA Configure    
	//pcie56_int_disable();
	result=pci_set_dma_mask(pcie56, DMA_BIT_MASK(32));
	 if(result!=0){
		PRINTK("<pcie56_init>: set mask Error return 0x%08X\n", ret);
	 }
  	result= pci_set_consistent_dma_mask(pcie56, DMA_BIT_MASK(32));
	 if(result!=0){
		PRINTK("<pcie56_init>: set mask 2 Error return 0x%08X\n", ret);
	 }
   	 pci_set_master(pcie56);
 /************************** tasklet_schedule ***********************************/
 //	DECLARE_TASKLET(mytasklet,pk_ok,0);
 set_mytasklet(pk_ok);
 
 PRINTK("<pcie56_init> before tasklet test\n");
 if(&mytasklet == NULL)
 	PRINTK("<pcie56_init> tasklet is NULL\n");
 else
	tasklet_schedule(&mytasklet);
...
// 	Init wait queue
	...
	init_waitqueue_head(&sfsendinq);
	init_waitqueue_head(&sfrecvoutq);
...
//	interrupt register/disable/enable
	...
	if(!recvtask){
	 ...
	 sfsendtask = kthread_run(sf_send_thread, NULL, "sf_send_kthread");
      if(!sendtask){
           ret = PTR_ERR(sfsendtask);
           goto fail_run_sfsendthread;
       }
	  sfrecvtask = kthread_run(sf_recv_thread, NULL, "sf_recv_kthread");
      if(!sendtask){
           ret = PTR_ERR(sfrecvtask);
           goto fail_run_sfrecvthread;
       } 
	...
	PRINTK("testflag sis %d\n",testflag);
	return SUCCESS;


	
fail_run_sfrecvthread:
	 kthread_stop(sfsendtask);


fail_run_sfsendthread:
